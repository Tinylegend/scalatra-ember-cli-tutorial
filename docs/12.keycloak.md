...
Option: You can chanage the routes/application.js, set the `session.set('onLoad', 'login-required');` to  `session.set('onLoad', 'check-sso');`. In this case, user can stay on logout without redirect to keycloak login page.

## Step 4: Sharing the keycloak login across the ember projects ## 
